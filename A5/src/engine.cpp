...
U16   Engine::minimax(const Board& b, int max_depth)
{
    Board temp_b(b);
    auto moveset=b.get_legal_moves();
    U16 best_move=0;
...
void Engine::find_best_move(const Board& b) {
    ...
    {
        max_ply=4;
        if(time_left.count()<30)
            max_ply=3;
        if(time_left.count()<15)
            max_ply=2;
        if(time_left.count()<5)
            max_ply=1;

        if(time_left.count()<=10)
        do_quescence=false;
    }
    ...
    this->best_move=minimax(b,max_ply);
    // do_quescence=true;
    // this->best_move=minimax(b,4);
