...
class cnf{
    ...
    int32_t n,k1,new_var;

    std::vector<std::vector<int32_t>> clauses;

    cnf(int32_t n, std::vector<std::vector<int32_t>>& adj, int32_t k1) : n(n), k1(k1){
        new_var = n + 1; // first n variables = G11,G12,...,G1n next n variables = G21,G22,...,G2n

        //####################### constraints ##################################
        full_connection(adj); // G1i and G1j => eij
